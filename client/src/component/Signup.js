...
const Login = (props) => {
  ...
  const handleLoginRecruiter = () => {
    // if(/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,16}$/.test(signupDetails.password)){
    ...
    Object.keys(inputErrorHandler).forEach((obj) => {
      if (inputErrorHandler[obj].required && inputErrorHandler[obj].untouched) {
        ...
        // console.log(`${obj[0].toUpperCase() + obj.substr(1)} is required`)
      } else {
        tmpErrorHandler[obj] = inputErrorHandler[obj];
      }
    });
          ...
          <PasswordInput
            ...
            onBlur={(event) => {
              if (event.target.value === "") {
                ...
                handleInputError("password", true, "Password Should contain atleast 6 Charecters ");
              }
              else if(!/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,16}$/.test(event.target.value)) {
                handleInputError("password", true, "Password Should contain atleast one Number and one Special Charecter");
              }
              else {
